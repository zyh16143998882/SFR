...
from data import ModelNet40PC
from viewGCN.tools.SmoothLoss import SmoothLoss
from models.color_pred import ColorPredict
...
from custom_dataset import collate_fn, ShapeNetCore, ScanObjectNN
from rotationNet.mvt_rotnet import RotationNet, AverageMeter, my_accuracy
from viewGCN.tools.Trainer_mvt import ModelNetTrainer_mvt_color
...
parser = argparse.ArgumentParser(description='MVAPF-PyTorch')

parser.add_argument('--data_dir', required=True, help='path to 3D dataset')
parser.add_argument('--run_mode', '-rmode', default="train",
                    choices=["train", "test_cls", "test_retr", "test_rot", "test_occ"],
...
parser.add_argument('--mvnetwork', '-m', default="mvcnn", choices=["mvcnn", "rotnet", "viewgcn", "viewgcn_raw"],
...
parser.add_argument('--views_config', '-s', default="circular",
                    choices=["circular", "random", "learned_circular", "learned_direct", "spherical",
                             "learned_spherical", "learned_random", "learned_transfer", "custom", "gcn"],
...
parser.add_argument('--dset_variant', '-dsetp', help='The variant used of the `ScanObjectNN` dataset  ',
                    default="hardest", choices=["obj_only", "with_bg", "hardest"])
...
parser.add_argument('--object_color', '-clr', default="custom",
                    choices=["white", "random", "black", "red", "green", "blue", "custom"],
                    help='the selection type of views ')
...
parser.add_argument('--depthmap', default="fix",
                        choices=["fix", "depth"],
                        help='the selection type of depth ')
parser.add_argument('--points_radius', default=0.006, type=float, help='points_radius')
parser.add_argument('--canonical_distance', default=1.0, type=float, help='distance between camera and pointcloud')
parser.add_argument('--first_stage_epochs', default=30, type=int, help='number of epochs for the first stage')
parser.add_argument('--no_joint_train', dest='no_joint_train',
                    action='store_true', help='if joint train when gcn')
parser.add_argument('--depth', default=18, type=int, help='resnet depth')
parser.add_argument('--use_avgpool', dest='use_avgpool',
                    action='store_true', help='if use_avgpool in dgcnn feature')
parser.add_argument('--seed', default=-1, type=int, help='seed')
parser.add_argument('--unuse_se', dest='unuse_se',
                    action='store_true', help='not use channel attention in LocalGCN')
...
setup = {**args, **config}
if setup["mvnetwork"] in ["rotnet", "mvcnn", "simpleview"]:
...
if "modelnet" in setup["data_dir"].lower():
    dset_train = ModelNet40PC(partition='train', num_points=1024, data_path='./data/modelnet40_ply_hdf5_2048/train_files.txt')
    dset_val = ModelNet40PC(partition='test', num_points=1024, data_path='./data/modelnet40_ply_hdf5_2048/test_files.txt')
...
train_loader = DataLoader(dset_train, batch_size=setup["batch_size"],
                          shuffle=True, num_workers=2, collate_fn=collate_fn, drop_last=True)
...
if setup["mvnetwork"] == "mvcnn":
    ...
    print('Using ' + setup["mvnetwork"] + str(setup["depth"]) + ' '+mvnetwork.agr_type)
...
if setup["mvnetwork"] == "viewgcn" or setup["mvnetwork"] == "viewdgcnn":
...
mvtn = MVTN(setup["nb_views"], views_config=setup["views_config"],
            ...
            shape_features_size=setup["features_size"], transform_distance=setup["transform_distance"],
            input_view_noise=setup["input_view_noise"], shape_extractor=setup["shape_extractor"],
            screatch_feature_extractor=setup["screatch_feature_extractor"])
color_predict = ColorPredict(shape_extractor="dgcnn", use_avgpool=setup["use_avgpool"])
print("shape_extractor == "+color_predict.shape_extractor)


mvrenderer = MVRenderer(nb_views=setup["nb_views"], image_size=setup["image_size"], pc_rendering=setup["pc_rendering"],
                        object_color=setup["object_color"], background_color=setup["background_color"],
                        faces_per_pixel=setup["faces_per_pixel"], points_radius=setup["points_radius"],
                        points_per_pixel=setup["points_per_pixel"], light_direction=setup["light_direction"],
                        cull_backfaces=setup["cull_backfaces"],depthmap=setup["depthmap"])

mvtn.to(DEVICE)
color_predict.to(DEVICE)
mvrenderer.to(DEVICE)
...
# color的优化器
if setup["object_color"] == "custom":
    color_optimizer = torch.optim.AdamW(color_predict.parameters(
    ), lr=setup["mvtn_learning_rate"], weight_decay=setup["mvtn_weight_decay"])
else:
...
models_bag = {"mvnetwork": mvnetwork, "optimizer": optimizer,
              ...
              "color": color_predict, "color_optimizer": color_optimizer}

if setup["seed"] == -1:
    setup["seed"]=np.random.randint(1, 10000)
print("seed == ",setup["seed"])
seed_torch(setup["seed"])
...
def train(data_loader, models_bag, setup):
    ...
    for i, (targets, meshes, points) in enumerate(data_loader):
        ...
        if setup["object_color"] == "custom":
            models_bag["color_optimizer"].zero_grad()
        if setup["is_learning_views"]:
        ...
        color = None
        if setup["object_color"] == "custom":
            color = models_bag["color"](points.permute(0, 2, 1))
        ...
        if VISUALIZER_PRE == True:
            for k in range(rendered_images.size(0)):
                for j in range(rendered_images.size(1)):
                    ...
                    vutils.save_image(img, VIS_REAL_PATH_POINT + '{}_{}_{}.jpg'.format(str(i),str(k), str(j)),
                                      ormalize=True)
        ...
        if setup["object_color"] == "custom":
            models_bag["color_optimizer"].step()
            if setup["clip_grads"]:
                clip_grads_(models_bag["color"].parameters(
...
def evluate(data_loader, models_bag, setup, is_test=False, retrieval=False):
    ...
    for i, (targets, meshes, points) in t:
        ...
        with torch.no_grad():
            ...
            loss = criterion(outputs, targets.long())
            c_views = ListDict(
                {"azim": azim.cpu().numpy().reshape(-1).tolist(), "elev": elev.cpu().numpy().reshape(-1).tolist(),
                 "dist": dist.cpu().numpy().reshape(-1).tolist(),
                 "label": np.repeat(targets.cpu().numpy(), setup["nb_views"]).tolist(),
...
def view_gcn_exp(setup, models_bag, train_loader, val_loader, dset_val):
    ...
    models_bag["color"].train()
    models_bag["mvrenderer"].train()

    trainer = ModelNetTrainer_mvt_color(models_bag, train_loader, val_loader, dset_val, SmoothLoss(
    ), 'svcnn', setup["checkpoint_dir1"], num_views=1, setup=setup, classes=classes, device=DEVICE)

    if setup["resume_first"]:
        # load_checkpoint(setup,models_bag,os.path.join(trainer.weights_dir, setup["exp_id"] + "_checkpoint.pt"))
    ...
    if setup["no_joint_train"]:
        models_bag["color"].eval()  # 前面是svcnn的所以在evaluate时不会设置为eval
    if setup["mvnetwork"] == "viewgcn":
        models_bag["mvnetwork"] = view_GCN(setup["exp_id"], models_bag["mvnetwork"], nclasses=len(classes),
                                           cnn_name=setup["cnn_name"],
                                           num_views=setup["nb_views"],unuse_se=setup["unuse_se"])  # 最开始的mvnetwork是第一阶段的，这里才是设置成第二阶段的view_GCN
    ...
    models_bag["mvnetwork"].to(DEVICE)

    if setup["mvnetwork"] == "viewgcn":
        trainer = ModelNetTrainer_mvt_color(models_bag, train_loader, val_loader, dset_val, SmoothLoss(),
                                            'view-gcn', setup["checkpoint_dir2"], num_views=setup["nb_views"], setup=setup,
                                            classes=classes, device=DEVICE)
    elif setup["mvnetwork"] == "viewdgcnn":
        trainer = ModelNetTrainer_mvt_color(models_bag, train_loader, val_loader, dset_val, SmoothLoss(),
                                            'view-dgcnn', setup["checkpoint_dir2"], num_views=setup["nb_views"],
                                            setup=setup,
                                            classes=classes, device=DEVICE)
    ...
    elif setup["viewgcn_phase"] == "all" or setup["viewgcn_phase"] == "second":
        if setup["run_mode"] == "train":
            trainer.train(setup["epochs"])
...
if setup["resume"] or "test" in setup["run_mode"]:
    if setup["mvnetwork"] in ["mvcnn", "rotnet"]:
        load_checkpoint_all(setup, models_bag, setup["weights_file"])
...
if setup["mvnetwork"] == "mvcnn":
    if setup["run_mode"] == "train":
        ...
        for epoch in range(setup["start_epoch"], n_epochs):
            ...
            saveables = {'epoch': epoch + 1,
                         ...
                         'color': models_bag["color"].state_dict() if setup["object_color"] == "custom" else None,
                         'acc': avg_test_acc,
                         'best_acc': setup["best_acc"],
                         'optimizer': models_bag["optimizer"].state_dict(),
                         'mvtn_optimizer': None if not setup["is_learning_views"] else models_bag[
            ...
            if (epoch + 1) % setup["plot_freq"] == 0:
                for indx, ii in enumerate(PLOT_SAMPLE_NBS):
                    ...
                    color = models_bag["color"](points[None, ...].permute(0, 2, 1))
    ...
    if setup["run_mode"] == "test_retr":
        ...
        mvtn = MVTN(setup["nb_views"], views_config=setup["views_config"],
                    ...
                    shape_features_size=setup["features_size"], transform_distance=setup["transform_distance"],
                    input_view_noise=setup["input_view_noise"], shape_extractor=setup["shape_extractor"],
                    screatch_feature_extractor=setup["screatch_feature_extractor"]).cuda()
...
elif setup["mvnetwork"] == "viewgcn" or setup["mvnetwork"] == "viewdgcnn":
