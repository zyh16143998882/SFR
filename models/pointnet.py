...
class PointNet2(nn.Module):
    def __init__(self, num_classes=40, alignment=False):
        ...
        #                       act=False, norm=False, bias=True, dropout=0)
...
#         local_feature = x  # this can be used in segmentation task. we comment it out here.
        ...
        # 2.3.10 classification
        # out = self.classifier(global_feature)
        return global_feature, global_feature, transform
...
class PointNet3(nn.Module):
    def __init__(self, num_classes=40, alignment=False):
        ...
        self.conv2s = Conv1dLayer(
            [64, 128, 512], act='relu', norm=True, bias=True)
...
#         local_feature = x  # this can be used in segmentation task. we comment it out here.
        ...
        # 2.3.10 classification
        return None, global_feature, transform



class PointNetColor(nn.Module):
    def __init__(self, alignment=False):
        super(PointNetColor, self).__init__()
        # look at the description under 2.2 or refer to the paper if you need more details
        ...
        self.conv3s = Conv1dLayer(
            [2048, 512, 64], act='relu', norm=True, bias=True)

        self.final = Conv1d(64, 3, 1, bias=True)
...
#         local_feature = x  # this can be used in segmentation task. we comment it out here.
        ...
        # input - B x 1024 x N
        # output - B x 1024
        global_feature = torch.max(x, dim=2, keepdim=True)[0].repeat(1,1,x.size(2))
        ...
        # 2.3.10 classification
        out = self.conv3s(torch.cat([global_feature,x],dim=1))
...
def load_point_ckpt2(model,  network_name,  ckpt_dir='./checkpoint', verbose=True):
    # ------------------ load ckpt
    filename = '{}/{}_model.pth'.format(ckpt_dir, network_name)
    if not os.path.exists(filename):
        print("No such checkpoint file as:  {}".format(filename))
