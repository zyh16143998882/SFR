...
from models.compositor2 import WeightedCompositor
...
class MVRenderer(nn.Module):
    ...
    def __init__(self, nb_views, image_size=224, pc_rendering=True, object_color="white", background_color="white", faces_per_pixel=1, points_radius=1,  points_per_pixel=1, light_direction="random", cull_backfaces=False, random_color=None, depthmap="fix"):
        super().__init__()
    ...
    def render_points(self, points, color, azim=None, elev=None, dist=None, background_color=(0.0, 0.0, 0.0), eyes_pos_list=None):
        c_batch_size = azim.shape[0]
        point_cloud = Pointclouds(points=points.to(torch.float), features=color * torch.ones_like(points, dtype=torch.float))

        if eyes_pos_list == None:
            R, T = look_at_view_transform(dist=batch_tensor(dist.T, dim=1, squeeze=True), elev=batch_tensor(
                elev.T, dim=1, squeeze=True), azim=batch_tensor(azim.T, dim=1, squeeze=True),)
        else:
            R, T = look_at_view_transform(eye=eyes_pos_list,
                                          at=torch.tensor([[0, 0, 0]], dtype=torch.float32).repeat(eyes_pos_list.size(0),1),
                                          up=torch.tensor([[0, 0, 1]], dtype=torch.float32).repeat(eyes_pos_list.size(0),1))
        R, T = check_and_correct_rotation_matrix(
            R, T, EXAHSTION_LIMIT, azim, elev, dist)
        ...
        elif self.depthmap == "depth":
            renderer = PointsRenderer(
                rasterizer=PointsRasterizer(
                    cameras=cameras, raster_settings=raster_settings),
                compositor=WeightedCompositor(
                    background_color=background_color)
            )
    ...
    def rendering_color(self, custom_color=(1.0, 0, 0), points=None):
        device = points.device
        if self.object_color == "custom":
            color = custom_color
        elif self.object_color == "random":
            color = self.random_color.repeat(points.size(0),1,1).to(device)
        elif self.object_color == "points":
            color = points
        else:
            color = torch_color(self.object_color, max_lightness=True,).to(device)
        return color
    ...
    def forward(self, meshes, points, azim, elev, dist, color=None, eyes_pos_list=None):
        """
        ...
        color = self.rendering_color(color, points)
        ...
        if not self.pc_rendering:
            ...
            rendered_images, cameras = self.render_points(
                points=points, color=color, azim=azim, elev=elev, dist=dist, background_color=background_color, eyes_pos_list=eyes_pos_list)
        return rendered_images, cameras

    def render_and_save(self, meshes, points, azim, elev, dist, images_path, cameras_path, color=None, eyes_pos_list=None):
        with torch.no_grad():
            rendered_images, cameras = self.forward(
                meshes, points, azim, elev, dist, color, eyes_pos_list)
