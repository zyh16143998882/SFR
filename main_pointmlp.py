...
from data_pointMLP import ModelNet40
...
from custom_dataset import collate_fn, ShapeNetCore
...
if "modelnet" in setup["data_dir"].lower():
    dset_train = ModelNet40(partition='train', num_points=setup["nb_points"])
    dset_val = ModelNet40(partition='test', num_points=setup["nb_points"])
    ...
    dset_train = ScanObjectNN(partition='training', num_points=setup["nb_points"])
    dset_val = ScanObjectNN(partition='test', num_points=setup["nb_points"])
    classes = ['bag', 'bed', 'bin', 'box', 'cabinet', 'chair', 'desk', 'display', 'door', 'pillow', 'shelf', 'sink', 'sofa', 'table', 'toilet']
...
if setup["mvnetwork"] == "viewgcn" or setup["mvnetwork"] == "fusenet" :
...
color_predict = ColorPredict(shape_extractor="dgcnn", use_avgpool=setup["use_avgpool"], output_channels=len(classes))
